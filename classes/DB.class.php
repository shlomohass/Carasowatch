...
class DB {
    ...
    public function __construct( $conf, $die = true) {
        ...
        $this->link = new mysqli( $conf['host'] , $conf['dbuser'] , $conf['dbpass'], $conf['dbname'], $conf['port'] ); 

        $this->link->set_charset( "utf8" );
        
        if( $this->link->connect_errno && $die)
        {
...
//INSERT TO DB:
    ...
    * @param array table column => column value
    * @param bool $log
    * @param bool $spit for advanced debugging echo query used.
    * @return bool
    */
    public function insert_safe( $table, $variables = array(), $log = true, $spit = false ) {
