...
class Func {
    ...
    */
    ...
    /** Filter an assoiative array by keys to exclude:
     * 
     * @param array $arr
     * @param array $_exckeys
     * @return array
     */
    public function get_array_without_keys($arr, $_exckeys) {
        $return = array();
        $exckeys = array_flip($_exckeys);
        foreach($arr as $key => $data) {
            if (!isset($exckeys[$key])) {
                $return[$key] = $data;
            }
        } 
        return $return;
    } 
    /** Filter an assoiative array by values to exclude:
     * 
     * @param array $arr
     * @param array $_excvalues
     * @return array
     */
    public function get_array_without_value($arr, $_excvalues) {
        $return = array();
        $excvalues = array_flip($_excvalues);
        foreach($arr as $key => $data) {
            if (!isset($excvalues[$data])) {
                $return[$key] = $data;
            }
        } 
        return $return;
    } 
    /** Filter an assoiative array by keys that includes a hash:
     * 
     * @param array $arr
     * @param string $hash
     * @return array
     */
    public function get_array_with_keys_include($arr, $hash) {
        $return = array();
        foreach($arr as $key => $data) {
